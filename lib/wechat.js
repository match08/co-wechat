...
class Wechat {
  constructor(config, isDebug, isLog) {
    ...
    return async (ctx, next) => {
      ...
      if (method === 'GET') {
        ...
        if (encrypted) {
          ...
          const nowSignature = CRYPTOR.getSignature(timestamp, nonce, echostr);
          valid = signature === nowSignature;
        } else {
          // 校验
          var signature = query.signature;
          const nowSignature  = getSignature(timestamp, nonce, TOKEN);
          valid = signature === nowSignature;
        }

        if(this.isLog)
        {
          console.log('[wechat] signature valid:', method, encrypted, valid, signature, nowSignature);
        }
        ...
        if (!valid) {
          ...
          if (encrypted) {
            ...
            if(this.isLog)
            {
              console.log('[wechat] appId valid:', method, encrypted, decrypted);
            }
          } else {
            ctx.body = echostr;
          }
...
module.exports = function (config, isDebug,isLog) {
  return new Wechat(config, isDebug, isLog);
};
